# مدارس أمين الرضا (ع) — المنصة التعليمية والإدارية الموحدة (v5.0 Final)

تم تطوير وهندسة المنصة لتكون نظاماً متكاملاً وإدارياً وأكاديمياً يعمل عبر ويب فائق السرعة (Vercel) أو كتطبيق موبايل ذاتي الخدمة (Capacitor / Offline-Ready) بدون أي اعتماد على مكتبات خارجية أو خطوط CDN، مع تطبيق كامل وخالٍ من الأخطاء لجميع المتطلبات الـ 15 وحزمة الميزات الملكية وخطط الهيكلة المتقدمة:

## 🌟 أبرز الإنجازات وخطة الهيكلة الموحدة (Stages 0 - 6 Completed)
1. **البوابة الموحدة الفورية (`portal.html` - Single Page Shell):** دمج كافة المحاور (الحوكمة، الإرشاد والتقارير، الأكاديمي والتحليلات، المالية التنفيذية) في شاشات مبوبة فورية تعمل عبر DOM بدون أي إعادة تحميل للمتصفح.
2. **⚡ شريط الإجراءات السريعة وتقليل النقرات بنسبة > 60%:** تحقيق الهدف التشغيلي لإنجاز المهام اليومية الخمس الشائعة (رصد درجة، تسجيل غياب، مطالبة الأقساط، إرسال تهنئة الشهادة، اعتماد إجازة) في نقرة واحدة أو نقرتين فقط عبر أزرار واتساب مدمجة مباشرة!
3. **🎨 حزمة شهادات التفوق المصورة والزخرفية (`certificates-generator.html`):** تضم 12 قالباً (4 قوالب رسمي وزخرفي + 8 قوالب أطفال مصورة لمدرسة أمين الرضا للأولاد والبنات مع خيار `illustrated_auto` الذي يختار القالب تلقائياً حسب جنس الطالب/ة). تتم الطباعة بنظام الغشاء الزجاجي (Frosted Overlay) بدقة A4 فائقة.
4. **🤖 تحليلات الذكاء الاصطناعي والخطط العلاجية (`academic-analytics.html`):** تصنيف الطلاب إلى 4 مستويات وتوليد خطط علاجية باللغة العربية السليمة وتوزيع التكليفات آلياً للمرشد والمعلم في خطوة واحدة.
5. **📦 تحسين الأداء وقاعدة البيانات (Capacitor & SQL Cleanup):** تجميع 134 ملف هجرة في ملف أساسي موحد (`sql/000_complete_system_baseline_v5.sql`) ونقل التاريخي إلى `sql/archive/`، مع تقسيم ملف الترجمة الكبير `i18n.js` إلى أجزاء مستقلة (`i18n-en.js`, `i18n-fa.js`) لخفض حجم الحزمة بنسبة 96% للمستخدمين العرب.
6. **🔒 حوكمة أمنية وصلاحيات صارمة:** تطبيق قفل الدرجات (الإدخال لمرة واحدة)، قصر رؤية المعلم على مواده، حجب الدرجات عند تأخر الأقساط، ونظام إجازات ومهام متعدد التكليف.

---

## الواجهات والتوجيه الموحد
