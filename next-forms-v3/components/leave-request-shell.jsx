...
function InputField({ field, locale, value, error, onChange, labelMap }) {
  ...
  const inputType = field.id === 'staff_email' ? 'email' : 'text';
...
function validateValues(template, values, labels, options = {}) {
  ...
  const startDate = values.leave_start_date;
  const endDate = values.leave_end_date;
  const returnDate = values.return_date;
  ...
  template.fields.forEach((field) => {
    ...
    if (field.type === 'file' && value) {
      ...
      if (requirePreparedUpload && field.id === 'supporting_document') {
  ...
  if (startDate && endDate && startDate > endDate) {
    errors.leave_end_date = labels.invalidDateRange;
  }
  if (endDate && returnDate && returnDate < endDate) {
    errors.return_date = labels.invalidReturnDate;
  }
...
export default function LeaveRequestShell({ locale, dictionary }) {
  const router = useRouter();
  const forms = dictionary.forms;
  const labels = forms.leaveRequest;
  const template = useMemo(() => buildTemplateByKey('leave_request'), []);
  ...
  useEffect(() => {
    ...
    const raw = window.localStorage.getItem('amin_forms_v3_leave_request_state');
    if (raw) {
      try {
  ...
  useEffect(() => {
    const timer = setInterval(async () => {
      ...
      setSaveState('saving');
      try {
        await saveDraftRpc({ form_slug: template.slug, locale: activeLocale, version_label: nextVersionStamp(), visibility: template.visibility, schema: template, form_values: values, autosave: true });
        setSaveState('saved');
  ...
  const validation = useMemo(() => validateValues(template, values, labels, { requirePreparedUpload: Boolean(values.supporting_document?.name && !uploadTicket) }), [template, values, labels, uploadTicket]);

  function persistLocal(nextValues, nextReceipt = receipt, nextUploadTicket = uploadTicket, nextAttachment = uploadedAttachment) {
    window.localStorage.setItem('amin_forms_v3_leave_request_state', JSON.stringify({ values: nextValues, receipt: nextReceipt, uploadTicket: nextUploadTicket, uploadedAttachment: nextAttachment }));
  }

  function setFieldValue(fieldId, value) {
    let normalizedValue = value;
    if (fieldId === 'supporting_document') {
      setFileObjects((current) => ({ ...current, supporting_document: value?.rawFile || null }));
      normalizedValue = value?.rawFile ? { name: value.rawFile.name, size: value.rawFile.size, type: value.rawFile.type, lastModified: value.rawFile.lastModified } : null;
    ...
    setValues((current) => {
      ...
      persistLocal(next, receipt, fieldId === 'supporting_document' ? null : uploadTicket, fieldId === 'supporting_document' ? null : uploadedAttachment);
      return next;
    });
  ...
  function resetForm() {
    ...
    setValues(next); setErrors({}); setReceipt(null); setUploadTicket(null); setUploadedAttachment(null); setFileObjects({}); setUploadState('idle'); persistLocal(next, null, null, null); setSubmitState('idle');
  }

  async function saveNow() {
    setSaveState('saving');
    try { await saveDraftRpc({ form_slug: template.slug, locale: activeLocale, version_label: nextVersionStamp(), visibility: template.visibility, schema: template, form_values: values, autosave: false }); persistLocal(values); setSaveState('saved'); }
    catch (error) { console.error(error); setSaveState('error'); }
  }

  async function prepareUploadTicket() {
    if (!values.supporting_document?.name) { setErrors((current) => ({ ...current, supporting_document: labels.requiredField })); return; }
    setUploadState('loading');
    try {
      const result = await requestUploadTicketRpc({ form_slug: template.slug, locale: activeLocale, field_id: 'supporting_document', file_name: values.supporting_document.name, content_type: values.supporting_document.type || 'application/octet-stream', byte_size: values.supporting_document.size || 0 });
      if (result?.ok === false) throw new Error(result.error || 'upload_ticket_failed');
      const ticketPayload = result?.data || result;
      const nextTicket = { ...ticketPayload, ticketId: ticketPayload?.ticket_id || ticketPayload?.upload_token || `UP-${Date.now()}`, expiresAtLabel: ticketPayload?.expires_at ? formatDateForLocale(activeLocale, String(ticketPayload.expires_at).slice(0,10)) : labels.uploadTicketPending };
      setUploadTicket(nextTicket); persistLocal(values, receipt, nextTicket, uploadedAttachment); setUploadState('ready'); setErrors((current) => ({ ...current, supporting_document: undefined }));
    } catch (error) { console.error(error); setUploadState('error'); setErrors((current) => ({ ...current, supporting_document: labels.uploadTicketError })); }
  }

  async function submitForm() {
    const needUploadTicket = Boolean(values.supporting_document?.name && !uploadTicket);
    const nextErrors = validateValues(template, values, labels, { requirePreparedUpload: needUploadTicket });
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length) { setSubmitState('validation_error'); return; }
    setSubmitState('submitting');
    const reportId = `LR-${Date.now()}`;
    try {
      let attachmentPayload = uploadedAttachment;
      if (values.supporting_document?.name) {
        if (!uploadTicket?.ticketId) { setErrors((current) => ({ ...current, supporting_document: labels.uploadTicketRequired })); setSubmitState('validation_error'); return; }
        if (!attachmentPayload) {
          const rawFile = fileObjects.supporting_document;
          if (!rawFile) { setErrors((current) => ({ ...current, supporting_document: labels.fileNeedsReselect })); setSubmitState('validation_error'); return; }
          setUploadState('uploading');
          const uploadResult = await uploadAttachmentTransport({ ticketId: uploadTicket.ticketId, formSlug: template.slug, fieldId: 'supporting_document', file: rawFile });
          if (uploadResult?.ok === false) throw new Error(uploadResult.error || 'upload_failed');
          attachmentPayload = uploadResult; setUploadedAttachment(uploadResult); setUploadState('uploaded');
        }
      }

      const result = await submitLeaveRequestRpc({
        ...
        uploaded_attachment: attachmentPayload ? { bucket: attachmentPayload.bucket, object_path: attachmentPayload.object_path, file_name: attachmentPayload.file_name, byte_size: attachmentPayload.byte_size } : null
      });
      if (result?.ok === false) throw new Error(result.error || 'submit_failed');

      const submittedAtIso = new Date().toISOString();
      const nextReceipt = { reportId, submittedAt: submittedAtIso, submittedAtLabel: formatDateForLocale(activeLocale, submittedAtIso.slice(0,10)), response: result };
      setReceipt(nextReceipt); persistLocal(values, nextReceipt, uploadTicket, attachmentPayload || uploadedAttachment); setSubmitState('submitted');
      const params = new URLSearchParams({ ref: reportId, submittedAt: submittedAtIso, locale: activeLocale, applicant: values.staff_name || '' });
      router.push(`/${activeLocale}/forms/leave-request/success?${params.toString()}`);
    } catch (error) { console.error(error); setSubmitState('submit_error'); setUploadState((current) => (current === 'uploading' ? 'error' : current)); }
  }

  const previewStaffRows = [
    { label: labels.fields.staff_name, value: values.staff_name },
    { label: labels.fields.staff_department, value: values.staff_department },
    { label: labels.fields.staff_role, value: values.staff_role },
    { label: labels.fields.staff_email, value: values.staff_email }
  ];
  const previewLeaveRows = [
    { label: labels.fields.leave_type, value: (template.fields.find((field) => field.id === 'leave_type')?.options || []).find((option) => option.value === values.leave_type)?.label?.[activeLocale] || '' },
    { label: labels.fields.leave_start_date, value: values.leave_start_date ? formatDateForLocale(activeLocale, values.leave_start_date) : '' },
    { label: labels.fields.leave_end_date, value: values.leave_end_date ? formatDateForLocale(activeLocale, values.leave_end_date) : '' },
    { label: labels.fields.return_date, value: values.return_date ? formatDateForLocale(activeLocale, values.return_date) : '' },
    { label: labels.fields.leave_reason, value: values.leave_reason }
  ];
      ...
      <section className="rounded-[30px] border border-slate-200 bg-white/90 p-5 shadow-soft">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-5">
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-sm"><StatusPill tone="brand">{meta.label}</StatusPill><StatusPill tone="slate">{localeDateLabel(activeLocale)}</StatusPill><StatusPill tone="slate">{labels.visibilityLabel}: {forms.visibility[template.visibility]}</StatusPill></div>
          </div>
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
          ...
          <aside className="space-y-4">
            ...
            <PreviewCard title={labels.staffPreviewTitle} rows={previewStaffRows} />
            <PreviewCard title={labels.leavePreviewTitle} rows={previewLeaveRows} />
            <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-soft"><h3 className="mb-3 text-lg font-black text-slate-950">{labels.printPreviewTitle}</h3><div className="mb-3 text-xs text-slate-500">{labels.printSheetHint}</div><div className="rounded-2xl border border-slate-100 bg-slate-50 p-3"><div className="mb-3 flex items-center justify-between gap-3 text-xs text-slate-500"><span>{labels.printPaperLabel}</span><StatusPill tone="slate">{template.printOrientation === 'landscape' ? forms.builder.printModes.landscape : forms.builder.printModes.portrait}</StatusPill></div><div className="scale-[0.97] origin-top"><PrintSheetPreview locale={activeLocale} template={template} values={values} /></div></div><div className="print-only mt-3 rounded-2xl border border-brand-100 bg-brand-50 px-4 py-3 text-sm text-brand-800">{labels.printReceiptBanner}</div></section>
            <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-soft no-print"><h3 className="mb-3 text-lg font-black text-slate-950">{labels.versionListTitle}</h3><div className="space-y-2 text-sm text-slate-600">{versions.length ? versions.map((version, index) => (<div key={version.version_id || version.version_label || index} className="rounded-2xl border border-slate-200 px-3 py-3"><div className="font-bold text-slate-900">{version.version_label || version.label || version.saved_at || 'version'}</div><div className="text-xs text-slate-500">{version.saved_at || version.source || 'rpc'}</div></div>)) : <div>{labels.noVersions}</div>}</div></section>
          </aside>
        </div>
      </section>
    </main>
