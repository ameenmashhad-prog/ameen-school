...
function switcher(){if(DATA.students.length<=1)return'';return `<select class="select" onchange="StudentHomeworks.setStudent(this.value)" aria-label="اختيار الطالب">${DATA.students.map(s=>`<option value="${s.id}" ${String(s.id)===String(CURRENT_STUDENT_ID)?'selected':''}>${esc(s.name)}</option>`).join('')}</select>`}
...
function homeworkCard(h){
  ...
  return `<article class="student-homework-card">
    ...
    <div class="homework-meta"><span>النشر: ${fmtDate(h.publish_at||h.assigned_date)}</span><span>التسليم: ${esc(h.due_date?dualDate(h.due_date):'—')} ${esc((h.due_time||'').slice(0,5))}</span><span>الدرجة: ${esc(h.max_score||10)}</span></div>
...
function homeworksView(){const list=scopedHomeworks();const pending=list.filter(h=>h.grade_score==null&&h.status==='published').length;setTimeout(markVisibleHomeworksViewed,300);$('#view-homeworks').innerHTML=`<div class="page-head"><div><h1>واجباتي</h1><p>الواجبات المنشورة والمغلقة مع المرفقات والدرجات.</p></div>${switcher()}</div><div class="kpis"><div class="kpi gold"><small>كل الواجبات</small><b>${list.length}</b></div><div class="kpi blue"><small>بانتظار درجة</small><b>${pending}</b></div><div class="kpi green"><small>مصححة</small><b>${list.filter(h=>h.grade_score!=null).length}</b></div><div class="kpi red"><small>متأخرة</small><b>${list.filter(isLate).length}</b></div></div>${quickLinksBar()}<div class="student-homework-list">${list.map(homeworkCard).join('')||emptyDebug()}</div>`}
function gradesView(){const rows=scopedHomeworks().filter(h=>h.grade_score!=null).map(h=>`<tr><td>${esc(h.title)}</td><td>${esc(h.subject_name||'—')}</td><td>${esc(h.grade_score)} / ${esc(h.grade_max_score||h.max_score)}</td><td>${esc(h.grade_feedback||'—')}</td><td>${fmtDate(h.graded_at)}</td></tr>`);$('#view-grades').innerHTML=`<div class="page-head"><div><h1>درجات الواجبات</h1><p>درجات الواجبات المصححة مع ملاحظات المعلم وتاريخ التصحيح.</p></div>${switcher()}</div>${quickLinksBar()}${table(['الواجب','المادة','الدرجة','ملاحظة المعلم','تاريخ التصحيح'],rows,'لا توجد درجات واجبات بعد')}`}
function table(h,rows,empty='لا توجد بيانات'){const body=Array.isArray(rows)?rows.join(''):String(rows||'');return body.trim()?`<div class="table-wrap"><table><thead><tr>${h.map(x=>`<th>${esc(x)}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table></div>`:emptyState(empty,'يمكنك تغيير الطالب أو العودة لاحقًا بعد ظهور بيانات جديدة.')}
function printView(){window.print()}
function bind(){$$('.nav button[data-view]').forEach(b=>b.addEventListener('click',()=>render(b.dataset.view)));$('#mobileMenuBtn')?.addEventListener('click',()=>$('#sidebar').classList.toggle('open'));$('#logoutBtn')?.addEventListener('click',async()=>{await client().auth.signOut({scope:'local'});location.href='index.html'});$('#refreshBtn')?.addEventListener('click',load);$('#printBtn')?.addEventListener('click',printView)}
async function init(){client();if(!await ensure())return;bind();await load()}
window.StudentHomeworks={init,render,setStudent,openAttachment,openSubmissionAttachment,addSubmissionFiles,removeSubmissionFile,saveSubmission,addComment,printView};
}());
