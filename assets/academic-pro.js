...
async function load(){const [classes,students,subjects,periods,weights,continuous,exams,scores,results,summary,decisions,locks,tasks]=await Promise.all([q('classes',{order:'name'}),q('students',{order:'name'}),q('subjects',{order:'name'}),q('academic_periods'),q('grade_weights'),q('continuous_assessments'),q('exams'),q('exam_scores'),q('v_academic_subject_results'),q('v_academic_student_summary'),q('academic_exemption_decisions',{order:'created_at',ascending:false}),q('academic_grade_locks',{order:'period_name'}),q('v_my_exam_submission_tasks',{order:'submission_deadline'})]);DATA={classes,students,subjects,periods,weights,continuous,exams,scores,results,summary,decisions,locks,tasks,classMap:new Map(classes.map(x=>[String(x.id),x])),studentMap:new Map(students.map(x=>[String(x.id),x])),subjectMap:new Map(subjects.map(x=>[String(x.id),x]))};render(ACTIVE)}
...
function renderSchedules() {
  ...
  const formHtml = `<div class="card" style="margin-bottom:20px"><div class="card-head"><h3>إعداد موعد امتحان وتكليف المعلم بتسليم الأسئلة (حصري بالإدارة)</h3></div><div class="card-body"><div class="academic-form">` +
  ...
  $('#view-schedules').innerHTML = `<div class="page-head"><div><h1>جدول ومواعيد الامتحانات ومهام الأسئلة 🗓️</h1><p>تحديد الإدارة لأيام ومواعيد الامتحانات حصراً، وتكليف المعلمين برفع الأسئلة ومتابعة التسليم.</p></div></div>` +
    ...
    formHtml + table(['الفترة', 'الصف', 'المادة', 'المعلم المكلف', 'موعد الامتحان', 'المادة المطلوبة', 'مهلة الرفع', 'الحالة', 'ملف الأسئلة', 'إجراء'], rows, 'لا توجد مواعيد امتحانات مسجلة');
}
