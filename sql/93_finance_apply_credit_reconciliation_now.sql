-- =============================================================
-- مدارس أمين الرضا (ع) — تطبيق تسوية الرصيد الدائن الآن
-- يحل تحذيرات:
-- overpaid_fees
-- fee_total_mismatches
-- بدون حذف أو إلغاء أي دفعة.
-- يحفظ الزيادة في student_fees.credit_balance.
-- =============================================================
...
begin
  ...
  if coalesce(p_apply,false) then
    ...
    set total_paid = target.new_total_paid,
        ...
        credit_notes = case
          when target.new_credit_balance > 0 then 'رصيد دائن ناتج عن مدفوعات زائدة محفوظة كسجل ولا يتم حذفها'
          else credit_notes
        end,
        updated_at = now()
    from target
  ...
  return jsonb_build_object(
    ...
    'preview', preview,
    'note', 'لا يتم حذف أو إلغاء أي دفعة. المدفوعات الزائدة تحفظ في student_fees.credit_balance كرصيد دائن.'
  );
end;
$$;
...
-- -------------------------------------------------------------
-- نسخة محدثة من الفحص المالي النهائي
-- -------------------------------------------------------------
...
begin
  ...
  -- بعد credit_balance لا تعد المدفوعات الزائدة مشكلة إذا total_paid نفسه لا يتجاوز الصافي.
  ...
  -- total_paid + credit_balance يجب أن يساوي مجموع الدفعات الفعلية غير الملغاة.
  select count(*) into fee_total_mismatches
  from public.student_fees sf
  where round(coalesce(sf.total_paid,0) + coalesce(sf.credit_balance,0),2) is distinct from round(coalesce((
    select sum(coalesce(fp.amount_usd,fp.amount,0))
    from public.fee_payments fp
...
-- تنفيذ التسوية الآن
-- -------------------------------------------------------------
select public.finance_reconcile_overpayments_to_credit(true, null) as applied_credit_reconciliation;

notify pgrst, 'reload schema';

select 'finance_credit_reconciliation_applied_now' as status;
