-- =============================================================
-- مدارس أمين الرضا (ع) — إصلاح قسري لخطأ View أعمدة صندوق اليومية
-- يحل نهائياً:
-- ERROR: cannot change name of view column "received_by_name" to "effective_receiver_name"
-- السبب: View قديمة موجودة بنفس الاسم وترتيب أعمدة مختلف.
-- الحل: DROP VIEW ... CASCADE ثم إعادة الإنشاء.
-- شغّل هذا الملف كاملاً من الحزمة، ولا تشغّل SQL 70 أو 71 بعده.
-- =============================================================
...
-- -------------------------------------------------------------
-- 1) الأعمدة المطلوبة في fee_payments
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- 3) الحذف القسري للـ Views القديمة
-- -------------------------------------------------------------
drop view if exists public.v_finance_cashbox_daily cascade;
drop view if exists public.v_fee_payments_detailed cascade;

-- -------------------------------------------------------------
-- 4) إعادة إنشاء View المدفوعات التفصيلية
-- -------------------------------------------------------------
create view public.v_fee_payments_detailed
...
where public.finance_can_manage()
   or exists(
      ...
      where st.id = sf.student_id
        and (st.user_id = auth.uid() or st.parent_id = auth.uid())
   );

grant select on public.v_fee_payments_detailed to authenticated;

-- -------------------------------------------------------------
-- 5) إعادة إنشاء View صندوق اليومية
-- -------------------------------------------------------------
create view public.v_finance_cashbox_daily
with (security_invoker=true) as
select
...
group by
  ...
  p.received_by,
  coalesce(p.receiver_name, rb.name, p.created_by_name, 'غير محدد'),
  coalesce(rb.role, p.receiver_role);

grant select on public.v_finance_cashbox_daily to authenticated;

-- -------------------------------------------------------------
-- 6) Payload صندوق اليومية
-- -------------------------------------------------------------
...
begin
  ...
  into rows_json
  from (
    select * from public.v_finance_cashbox_daily
...
select 'finance_cashbox_force_drop_views_ready' as status;
