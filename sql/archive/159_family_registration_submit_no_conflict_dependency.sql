-- ============================================================================
-- SAFER PATCH — Family Registration submit without ON CONFLICT dependency
--
-- استخدم هذا الملف إذا أردت تجاوز مشكلة unique index / ON CONFLICT نهائياً.
-- هذا الملف لا يعتمد على وجود unique constraint على forms_v3_submission_ref.
-- بل يعمل بمنطق:
--   - إذا وجد family بنفس submission_ref → update
--   - إذا لم يوجد → insert
-- ============================================================================
...
alter table public.registration_families
  add column if not exists general_family_health_notes text;

alter table public.registration_families
  add column if not exists document_notes text;

alter table public.registration_families
  add column if not exists finance_payment_entries jsonb not null default '[]'::jsonb;

alter table public.registration_families
  add column if not exists family_attachment jsonb;

alter table public.registration_students add column if not exists previous_school text;
alter table public.registration_students add column if not exists address_mashhad text;
alter table public.registration_students add column if not exists address_iraq text;
alter table public.registration_students add column if not exists health_notes text;
alter table public.registration_students add column if not exists student_class_label text;
alter table public.registration_students add column if not exists fee_structure_id uuid null references public.fee_structures(id) on delete set null;
alter table public.registration_students add column if not exists annual_fee_snapshot numeric not null default 0;
alter table public.registration_students add column if not exists monthly_fee_snapshot numeric not null default 0;
alter table public.registration_students add column if not exists finance_plan_type text default 'monthly';
alter table public.registration_students add column if not exists finance_installments_count int;
alter table public.registration_students add column if not exists finance_plan_start_date date;
alter table public.registration_students add column if not exists finance_plan_end_date date;
alter table public.registration_students add column if not exists finance_installment_schedule jsonb not null default '[]'::jsonb;
