-- ============================================================================
-- Forms v3 — Family Registration finance bridge
-- الهدف:
-- 1) تخزين طلب family-registration-v3 داخل جداول التسجيل القديمة أيضاً
-- 2) حفظ snapshot الرسوم السنوية وخطة الأقساط لكل طالب
-- 3) إبقاء forms_v3_submissions كمصدر review حديث
-- ============================================================================

alter table public.registration_families add column if not exists forms_v3_submission_ref text;
alter table public.registration_families add column if not exists general_family_health_notes text;
alter table public.registration_families add column if not exists document_notes text;
alter table public.registration_families add column if not exists finance_payment_entries jsonb not null default '[]'::jsonb;
alter table public.registration_families add column if not exists family_attachment jsonb;
