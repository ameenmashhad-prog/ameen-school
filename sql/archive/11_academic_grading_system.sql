-- =============================================================
-- مدارس أمين الرضا (ع) — نظام الدرجات والتقييم الأكاديمي
-- المراحل: ابتدائي، متوسط، إعدادي
-- الحسابات تلقائية عبر Views و Functions.
-- =============================================================
...
create table if not exists public.continuous_assessments (
  ...
  component_type text not null check (component_type in ('participation','homework','activity','project','discipline','quiz','other')),
...
create table if not exists public.exams (
  ...
  exam_type text not null default 'monthly' check (exam_type in ('monthly','extra','final')),
  exam_order int,
  max_score numeric not null default 100,
  exam_date date,
  created_by uuid null default auth.uid(),
...
create table if not exists public.academic_exemption_decisions (
  ...
  exemption_kind text not null check (exemption_kind in ('none','subject','general','candidate','review','cancelled')),
...
-- -------------------------------------------------------------
-- 5) مخالفات أكاديمية تمنع الإعفاء
-- -------------------------------------------------------------
...
-- أعمدة توافق مع الجداول القديمة للدرجات والسلوك حتى لا تفشل Views عند اختلاف المخطط.
...
-- -------------------------------------------------------------
-- 6) دالة تحديد المرحلة من اسم الصف
-- -------------------------------------------------------------
...
-- حذف Views القديمة قبل إعادة إنشائها لتجنب اختلاف الأعمدة.
...
-- -------------------------------------------------------------
-- 7) View نتائج المادة لكل طالب
-- -------------------------------------------------------------
create or replace view public.v_academic_subject_results
...
-- -------------------------------------------------------------
-- 8) View ملخص الطالب العام والإعفاء العام
-- -------------------------------------------------------------
create or replace view public.v_academic_student_summary
with (security_invoker=true) as
select
  r.student_id,
  r.student_name,
