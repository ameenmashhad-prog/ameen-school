-- =============================================================
-- مدارس أمين الرضا (ع) — Hotfix صندوق اليومية
-- يحل خطأ: cannot change name of view column received_by_name to effective_receiver_name
-- السبب: v_fee_payments_detailed كانت موجودة بأعمدة قديمة. الحل: حذف الـ Views وإعادة إنشائها.
-- =============================================================
...
-- -------------------------------------------------------------
-- 3) حذف Views القديمة ثم إعادة إنشائها بالترتيب الصحيح
-- -------------------------------------------------------------
drop view if exists public.v_finance_cashbox_daily;
drop view if exists public.v_fee_payments_detailed;
...
select 'finance_cashbox_view_recreate_fix_ready' as status;
