-- ============================================================================
-- Family Registration v3 — admin/accountant context + extra student attachments
-- الهدف:
-- 1) حفظ صفة مقدم الطلب واسمه داخل registration_families
-- 2) حفظ اسم مستلم الدفعة / المحاسب وسياق المراجعة الإدارية
-- 3) حفظ مرفقات إضافية للطالب (صورة الجواز + التسلسل/الشهادات)
-- 4) إبقاء النموذج متوافقاً مع النظام الحالي دون كسر التفعيل أو المالية
-- ============================================================================

alter table public.registration_families add column if not exists applicant_relation text;
alter table public.registration_families add column if not exists applicant_other_relation text;
alter table public.registration_families add column if not exists applicant_name text;
alter table public.registration_families add column if not exists accountant_receiver_name text;
alter table public.registration_families add column if not exists admin_reviewer_name text;
alter table public.registration_families add column if not exists admin_reviewer_id uuid null references public.users(id) on delete set null;
alter table public.registration_families add column if not exists admin_reviewed_at timestamptz;
alter table public.registration_families add column if not exists finance_receiver_name text;
alter table public.registration_families add column if not exists finance_receiver_id uuid null references public.users(id) on delete set null;
alter table public.registration_families add column if not exists finance_received_at timestamptz;

alter table public.registration_students add column if not exists passport_attachment jsonb;
alter table public.registration_students add column if not exists academic_documents_attachment jsonb;
...
begin
  ...
  where forms_v3_submission_ref = v_ref
  ...
  if v_family_id is null then
    insert into public.registration_families(
      ...
      family_attachment,
      applicant_relation,
      applicant_other_relation,
      applicant_name,
      accountant_receiver_name,
      ...
      nullif(v_approval->>'applicant_relation', ''),
      nullif(v_approval->>'applicant_other_relation', ''),
      nullif(v_approval->>'applicant_name', ''),
      nullif(v_approval->>'accountant_receiver_name', ''),
      v_ref,
      'pending',
      auth.uid(),
      now()
    ...
    set academic_year = coalesce(v_guardian->>'finance_academic_year', academic_year),
        guardian_name = coalesce(nullif(p_values->>'guardian_full_name', ''), nullif(p_values->>'guardian_name', ''), guardian_name),
        father_name = nullif(v_guardian->>'guardian_father_name', ''),
        family_name = nullif(p_values->>'family_name', ''),
        generated_username = nullif(v_guardian->>'guardian_username', ''),
        birth_date = v_guardian_birth,
        passport_number = nullif(v_guardian->>'guardian_passport_number', ''),
        nationality = nullif(v_guardian->>'guardian_nationality', ''),
        phone_primary = nullif(v_guardian->>'guardian_phone_primary', ''),
        phone_whatsapp = nullif(v_guardian->>'guardian_phone_whatsapp', ''),
        phone_emergency = nullif(v_guardian->>'guardian_phone_emergency', ''),
        education_level = nullif(v_guardian->>'guardian_education_level', ''),
        education_notes = nullif(v_guardian->>'guardian_education_notes', ''),
        work_type = nullif(v_guardian->>'guardian_work_type', ''),
        work_notes = nullif(v_guardian->>'guardian_work_notes', ''),
        residence_type = nullif(v_guardian->>'residence_type', ''),
        mother_name = nullif(v_mother->>'mother_given_name', ''),
        mother_father_name = nullif(v_mother->>'mother_father_name', ''),
        mother_family_name = nullif(v_mother->>'mother_family_name', ''),
        mother_birth_date = v_mother_birth,
        mother_passport_number = nullif(v_mother->>'mother_passport_number', ''),
        mother_nationality = nullif(v_mother->>'mother_nationality', ''),
        mother_phone = nullif(v_mother->>'mother_phone', ''),
        mother_whatsapp = nullif(v_mother->>'mother_whatsapp', ''),
        mother_education_level = nullif(v_mother->>'mother_education_level', ''),
        mother_education_notes = nullif(v_mother->>'mother_education_notes', ''),
        mother_work_type = nullif(v_mother->>'mother_work_type', ''),
        mother_work_notes = nullif(v_mother->>'mother_work_notes', ''),
        general_family_health_notes = nullif(v_context->>'general_family_health_notes', ''),
        document_notes = nullif(v_documents->>'document_notes', ''),
        finance_payment_entries = coalesce(p_values->'payment_entries', '[]'::jsonb),
        family_attachment = coalesce(v_documents->'family_attachment', p_uploaded_attachment, family_attachment),
        applicant_relation = nullif(v_approval->>'applicant_relation', ''),
        applicant_other_relation = nullif(v_approval->>'applicant_other_relation', ''),
        applicant_name = nullif(v_approval->>'applicant_name', ''),
        accountant_receiver_name = nullif(v_approval->>'accountant_receiver_name', ''),
        status = 'pending',
        updated_at = now()
