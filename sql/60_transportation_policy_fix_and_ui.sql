-- =============================================================
-- مدارس أمين الرضا (ع) — إصلاح وحدة النقل المدرسي + سياسات بدون أعمدة مبهمة
...
-- -------------------------------------------------------------
-- 2) RLS + helper functions بدون أي أعمدة مبهمة
-- -------------------------------------------------------------
create or replace function public.transport_user_can_read_student(p_student_id uuid)
returns boolean
language sql
security definer
set search_path = public
stable
as $$
  select public.transport_can_manage()
    or exists(
...
grant execute on function public.transport_user_can_read_student(uuid) to authenticated;

create or replace function public.transport_user_can_read_route(p_route_id uuid)
returns boolean
language sql
security definer
set search_path = public
...
grant execute on function public.transport_user_can_read_route(uuid) to authenticated;

create or replace function public.transport_user_can_read_trip(p_trip_id uuid)
returns boolean
language sql
security definer
set search_path = public
  ...
  select public.transport_can_manage()
    or exists(
      ...
      where t.id = p_trip_id
        and public.transport_user_can_read_route(t.route_id)
    );
$$;

grant execute on function public.transport_user_can_read_trip(uuid) to authenticated;
...
create policy transport_drivers_manage on public.transport_drivers
  for all to authenticated
  using (public.transport_can_manage() or public.transport_drivers.user_id = auth.uid())
  with check (public.transport_can_manage());

create policy transport_routes_read_manage on public.transport_routes
  for all to authenticated
  using (public.transport_user_can_read_route(public.transport_routes.id))
  with check (public.transport_can_manage());

create policy transport_stops_read_manage on public.transport_route_stops
  for all to authenticated
  using (public.transport_user_can_read_route(public.transport_route_stops.route_id))
  with check (public.transport_can_manage());

create policy transport_assignments_scoped on public.transport_student_assignments
  for all to authenticated
  using (public.transport_can_manage() or public.transport_user_can_read_student(public.transport_student_assignments.student_id))
  with check (public.transport_can_manage());

create policy transport_trips_manage on public.transport_trips
  for all to authenticated
  using (public.transport_user_can_read_trip(public.transport_trips.id))
  with check (public.transport_can_manage());

create policy transport_trip_attendance_scoped on public.transport_trip_attendance
  for all to authenticated
  using (public.transport_user_can_read_trip(public.transport_trip_attendance.trip_id) or public.transport_user_can_read_student(public.transport_trip_attendance.student_id))
  with check (public.transport_can_manage());
...
select 'transportation_management_policy_fix_ready' as status;
